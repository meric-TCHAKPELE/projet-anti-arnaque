...
if model is None:
    ...
    # Zone de saisie
    user_input = st.text_area("Quel est le message que vous avez reçu ? :", height=150, placeholder="Ex: Urgent virement, vous avez gagné...")

    if st.button("Vérifions maintenant!"):
        if user_input.strip():
            # Traitement
            ...
            if str(prediction).lower() in ['arnaque', '1', '1.0']:
                st.error("### ⚠️ ce message pourrait être une arnaque")
